...
#include <s2e/Plugins/uEmu/RegisterLogger.h>
#include <s2e/Plugins/uEmu/RegisterLogPlotter.h>
...
private:
    ...
    sigc::connection onEngineShutdownConnection;
    InvalidStatesDetection *onInvalidStateDectionConnection;
    ARMFunctionMonitor *onARMFunctionConnection;
    RegisterLogger m_registerLogger;
    RegisterLogPlotter m_registerLogPlotter;
    std::string m_registerLogPath;
    bool m_registerLoggerFlushed;
    bool m_enableRegisterPlots;
    unsigned m_registerPlotTopN;
    std::string m_registerPlotInputCsv;
    std::string m_registerPlotOutputDir;
    std::string m_registerPlotOutputPrefix;
    ...
    RegisterLogger::RegisterCategory getRegisterCategory(S2EExecutionState *state, uint32_t phaddr, uint32_t pc);
    RegisterLogger::RegisterCategory getCachedRegisterCategory(uint32_t phaddr, uint32_t pc) const;
    void updateRegisterCategory(uint32_t phaddr, RegisterLogger::RegisterCategory category);
    RegisterLogger::RegisterCategory getFinalLearningCategory(S2EExecutionState *state, uint32_t phaddr);
    void finalizeRegisterCategories(S2EExecutionState *state);
    void flushRegisterLogger();
    void generateRegisterPlots();
