...
void PeripheralModelLearning::initialize() {
    ...
    durationtime = 0;
    all_peripheral_no = 0;
    m_registerLoggerFlushed = false;
    firmwareName = s2e()->getConfig()->getString(getConfigKey() + ".firmwareName", "x.elf");
    std::size_t fwIndex = firmwareName.find_last_of("/\\");
    std::string fwName = fwIndex == std::string::npos ? firmwareName : firmwareName.substr(fwIndex + 1);
    m_registerLogPath = s2e()->getOutputDirectory() + "/" + fwName + "-registers-dr-sr.csv";
    m_enableRegisterPlots = s2e()->getConfig()->getBool(getConfigKey() + ".enableRegisterPlots", true);
    int registerPlotTopN = s2e()->getConfig()->getInt(getConfigKey() + ".registerPlotTopNBaseAddresses", 10);
    m_registerPlotInputCsv = s2e()->getConfig()->getString(getConfigKey() + ".registerPlotInputCsv", "");
    m_registerPlotOutputDir = s2e()->getConfig()->getString(getConfigKey() + ".registerPlotOutputDir",
                                                             s2e()->getOutputDirectory());
    m_registerPlotOutputPrefix = s2e()->getConfig()->getString(getConfigKey() + ".registerPlotOutputPrefix", "");

    if (registerPlotTopN <= 0) {
        registerPlotTopN = 10;
    }
    m_registerPlotTopN = static_cast<unsigned>(registerPlotTopN);

    if (m_registerPlotInputCsv.empty()) {
        m_registerPlotInputCsv = m_registerLogPath;
    }
    ...
    onEngineShutdownConnection = s2e()->getCorePlugin()->onEngineShutdown.connect(
        sigc::mem_fun(*this, &PeripheralModelLearning::onEngineShutdown));
...
RegisterLogger::RegisterCategory PeripheralModelLearning::getRegisterCategory(S2EExecutionState *state, uint32_t phaddr,
                                                                              uint32_t pc) {
    DECLARE_PLUGINSTATE(PeripheralModelLearningState, state);
    TypeFlagPeripheralMap typeFlags = plgState->get_type_flag_phs();
    auto it = typeFlags.find(phaddr);
    if (it == typeFlags.end()) {
        auto irqIt = irq_data_phs.find(phaddr);
        if (irqIt != irq_data_phs.end() && irqIt->second == 2) {
            return RegisterLogger::RegisterCategory::DR;
        }
        return RegisterLogger::RegisterCategory::Unknown;
    }
    ...
    if (it->second == T1) {
        ...
        return RegisterLogger::RegisterCategory::SR;
    }

    return RegisterLogger::RegisterCategory::CR;
}

RegisterLogger::RegisterCategory PeripheralModelLearning::getCachedRegisterCategory(uint32_t phaddr, uint32_t pc) const {
    auto it = cache_type_flag_phs.find(phaddr);
    if (it == cache_type_flag_phs.end()) {
        auto irqIt = irq_data_phs.find(phaddr);
        if (irqIt != irq_data_phs.end() && irqIt->second == 2) {
            return RegisterLogger::RegisterCategory::DR;
        }
...
void PeripheralModelLearning::flushRegisterLogger() {
    if (m_registerLoggerFlushed || m_registerLogPath.empty()) {
        return;
    }

    m_registerLogger.flushToCsv(m_registerLogPath);
    m_registerLoggerFlushed = true;
    getInfoStream() << "Register logger output saved to " << m_registerLogPath << "\n";
}

void PeripheralModelLearning::generateRegisterPlots() {
    if (!m_enableRegisterPlots) {
        return;
    }

    RegisterLogPlotter::Options options;
    options.inputCsv = m_registerPlotInputCsv;
    options.outputDir = m_registerPlotOutputDir;
    options.outputPrefix = m_registerPlotOutputPrefix;
    options.topNBaseAddresses = m_registerPlotTopN;

    std::vector<std::string> generatedPlots;
    std::string result;
    if (!m_registerLogPlotter.generate(options, generatedPlots, result)) {
        getWarningsStream() << "Register plot generation failed: " << result << "\n";
        return;
    }

    if (!result.empty()) {
        getInfoStream() << "Register plotter: " << result << "\n";
    }

    for (const auto &plotPath : generatedPlots) {
        getInfoStream() << "Register plot saved to " << plotPath << "\n";
    }
}

void PeripheralModelLearning::updateRegisterCategory(uint32_t phaddr, RegisterLogger::RegisterCategory category) {
    m_registerLogger.updateCategory(phaddr, category);
    getDebugStream() << "Updated register category for " << hexval(phaddr) << " to " 
                     << (category == RegisterLogger::RegisterCategory::DR ? "DR" : "SR") << "\n";
}

RegisterLogger::RegisterCategory PeripheralModelLearning::getFinalLearningCategory(S2EExecutionState *state,
                                                                                    uint32_t phaddr) {
    DECLARE_PLUGINSTATE(PeripheralModelLearningState, state);

    auto readCache = plgState->get_readphs();
    auto readIt = readCache.find(phaddr);
    uint32_t readCount = readIt == readCache.end() ? 0 : readIt->second.second;

    uint32_t typeFlag = plgState->get_type_flag_ph_it(phaddr);
    if (plgState->get_t3_type_ph_size(phaddr) > 0) {
        return RegisterLogger::RegisterCategory::DR;
    }
    if (typeFlag == T3) {
        return RegisterLogger::RegisterCategory::DR;
    ...
    if (irq_data_phs[phaddr] == 1) {
        return RegisterLogger::RegisterCategory::SR;
    }

    if (plgState->get_lock_t1_type_flag(phaddr) == 1) {
        if (PT1_phs[phaddr] == 1 && mulit_value_dr[phaddr] == 1 && readCount > 0x30 && !enable_fuzzing) {
            return RegisterLogger::RegisterCategory::DR;
        }
        return RegisterLogger::RegisterCategory::SR;
    }

    if (plgState->get_dt1_type_flag_ph_it(phaddr) == 1 && readCount > 0x10 && readCount < 200 && !enable_fuzzing) {
        return RegisterLogger::RegisterCategory::DR;
    }
...
void PeripheralModelLearning::finalizeRegisterCategories(S2EExecutionState *state) {
    if (!state) {
        return;
    }
    ...
    auto loggedAddresses = m_registerLogger.getLoggedAddresses();
    for (auto phaddr : loggedAddresses) {
        allAddresses.insert(phaddr);
    }

    auto typeFlags = plgState->get_type_flag_phs();
    for (const auto &it : typeFlags) {
        allAddresses.insert(it.first);
    }

    auto readCache = plgState->get_readphs();
    for (const auto &it : readCache) {
        allAddresses.insert(it.first);
    }

    auto t3Cache = plgState->get_t3_type_phs();
    for (const auto &it : t3Cache) {
        allAddresses.insert(it.first);
    }

    for (auto phaddr : allAddresses) {
        auto finalCategory = getFinalLearningCategory(state, phaddr);
        if (finalCategory != RegisterLogger::RegisterCategory::Unknown) {
            bool isRead = false;
            auto readIt = readCache.find(phaddr);
            if (readIt != readCache.end() && readIt->second.second > 0) {
                isRead = true;
            }

            m_registerLogger.upsert(RegisterLogger::computeBaseAddress(phaddr), phaddr, isRead, false, finalCategory);
        }
    }
}
    ...
    while (getline(fPHKB, peripheralcache)) {
        ...
        if (getGeneralEntryfromKB(peripheralcache, &type, &phaddr, &pc, &value, &cwirq_value)) {
            ...
            if (type == T0) {
                ...
                updateRegisterCategory(phaddr, RegisterLogger::RegisterCategory::SR);
                ...
                updateRegisterCategory(phaddr, RegisterLogger::RegisterCategory::DR);
...
klee::ref<klee::Expr> PeripheralModelLearning::onLearningMode(S2EExecutionState *state, SymbolicHardwareAccessType type,
    ...
    RegisterLogger::RegisterCategory regCat = getRegisterCategory(state, phaddr, pc);
    uint32_t baseAddr = RegisterLogger::computeBaseAddress(phaddr);
    m_registerLogger.logRead(baseAddr, phaddr, regCat);
...
klee::ref<klee::Expr> PeripheralModelLearning::onFuzzingMode(S2EExecutionState *state, SymbolicHardwareAccessType type,
    ...
    RegisterLogger::RegisterCategory regCat = getCachedRegisterCategory(phaddr, pc);
    uint32_t baseAddr = RegisterLogger::computeBaseAddress(phaddr);
    m_registerLogger.logRead(baseAddr, phaddr, regCat);
...
void PeripheralModelLearning::onWritePeripheral(S2EExecutionState *state, uint64_t phaddr,
    ...
    uint32_t pc = state->regs()->getPc();
    RegisterLogger::RegisterCategory regCat = getRegisterCategory(state, (uint32_t)phaddr, pc);
    uint32_t baseAddr = RegisterLogger::computeBaseAddress((uint32_t)phaddr);
    m_registerLogger.logWrite(baseAddr, (uint32_t)phaddr, regCat);
...
void PeripheralModelLearning::onLearningTerminationDetection(S2EExecutionState *state, bool *actual_end,
    ...
    if (enable_fuzzing && auto_mode_switch) {
        ...
        finalizeRegisterCategories(state);
                        ...
                        if (itt1 != t1_type_phs.end()) { // deal with t1
                            if (itt1->second.second.second != itch.second.second
                                ...
                                if (itt1->second.first == itch.first) {
                                    ...
                                    updateRegisterCategory(it.first.first, RegisterLogger::RegisterCategory::DR);
                                    getInfoStream(state)
...
void PeripheralModelLearning::onEngineShutdown() {
    flushRegisterLogger();
    generateRegisterPlots();
}
